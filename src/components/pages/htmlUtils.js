 ...
 */
export const normalizeHtmlTags = (html) => {
  if (!html) return '';
  return html
    .replace(/<\/?H1>/gi, (tag) => tag.toLowerCase())
    .replace(/<\/?H2>/gi, (tag) => tag.toLowerCase())
    .replace(/<\/?H3>/gi, (tag) => tag.toLowerCase())
    .replace(/<\/?H4>/gi, (tag) => tag.toLowerCase())
    .replace(/<\/?H5>/gi, (tag) => tag.toLowerCase());
};
 ...
 */
export const extractSection = (description, sectionTitles) => {
  ...
  const titleGroup = sectionTitles.map(t => t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
 ...
 */
export const formatCourseContent = (content) => {
  ...
  return sections.map(section => {    // Process numbered lists within each section
    if (section.match(/(\d+\.\s+[^\n]+|^\s*-\s+[^\n]+)/gm)) {
      // If we detect a numbered list pattern or dash list pattern, process it
      const lines = section.split('\n');
      let inList = false;
      let processedSection = '';      lines.forEach(line => {
