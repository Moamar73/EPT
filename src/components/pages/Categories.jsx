  ...
  useEffect(() => {
    const fetchCategoriesAndSubs = async () => {
      try {
        ...
        const mainCategoriesResponse = await fetch(`${import.meta.env.VITE_API_LINK}/api/courses/main-category`);
        const mainCategoriesData = await mainCategoriesResponse.json();
        const categoryNames = mainCategoriesData.map((cat) => cat.category);

        // Fetch subcategories for each main category
        const categoriesWithSubs = await Promise.all(
          categoryNames.map(async (categoryName, index) => {
            try {
              const subCategoriesResponse = await fetch(
                `${import.meta.env.VITE_API_LINK}/api/courses/category/${encodeURIComponent(categoryName)}`
              );
              const subCategoriesData = await subCategoriesResponse.json();
              const subCategoryList = subCategoriesData.map((item) => item.sub_category);
            ...
            {loading ? (
              ...
              <div className="text-center py-10">
                <p className="text-gray-600">No categories available at the moment.</p>
                  ...
                  {categories.map(({category, subs, color}, i) => (
