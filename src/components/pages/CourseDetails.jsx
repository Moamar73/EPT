...
const CourseDetails = () => {
  ...
  // Get date from React Router state (passed from Courses component)
  ...
  const selectedDate = selectedDateFromState 
    ? format(selectedDateFromState, 'yyyy-MM-dd')
    : null;
  
  // Get location from URL parameters (still visible)
    ...
    const fetchCourseData = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_LINK}/api/courses/name/${name}`
        );
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_LINK}/api/courses/main-category`
        );
  ...
  useEffect(() => {
    const fetchGalleryItems = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_LINK}/api/gallery`);
  ...
  const handleSearchSubmit = (e) => {
    ...
    if (searchForm.level && searchForm.level !== "Choose a Date") {
      ...
      const level = levelMapping[searchForm.level];
      if (level) params.set("level", level);
    }

    // Navigate to courses page with search parameters
    ...
    <div className="max-w-screen-2xl mx-auto font-times">
      ...
      <div className="container-px flex max-lg:flex-col xl:gap-11 gap-9 py-10">
        <div className="course-details flex-grow xl:text-[20px] lg:text-[16.5px]">
          ...
          <div
            className="mt-2 mb-10 rounded-lg shadow-sm"
            ref={triggerRef}
          >
            <p className="mb-5">
              This course based on supporting the employee in the following competencies, at advanced rates, which are:
            </p>
            <div className="font-goldman text-black flex flex-col gap-y-[8px] max-xs:text-[11.5px] mt-4">
          ...
          {whoShouldAttend && (
            <div className="mb-8">
              <h2 className="text-2xl font-bold mb-4 text-red">
                Who Should Attend
              </h2>
              <div
                className="course-content-section"
          ...
          {objectives && (
            <div className="mb-8">
              <h2 className="text-2xl font-bold mb-4 text-red">Objectives</h2>
              <div
                className="course-content-section"
                dangerouslySetInnerHTML={{
          ...
          <div className="bg-darkRed xl:h-[75px] h-[65px] flex min-[681px]:justify-between max-[680px]:relative items-center gap-3 max-[680px]:mb-[70px] max-[680px]:h-[90px] mt-10">
            ...
            <span className="font-bold text-white xl:text-[18px] min-[1074px]:text-[17px] text-[14.9px] text-center max-[680px]:pe-3">
              ...
              <span className="text-yellow">NAME</span> in 5 seconds
            </span>
       ...
       { /* Sidebar */}
          <div className="xl:min-w-[350px] lg:min-w-[290px]">
            <div className="shadow-custom rounded-[7px] border border-[#D0D0D0] overflow-hidden max-w-[592px] mx-auto">
              <div className="bg-blue text-white font-goldman px-4 py-[6px] mb-3 max-lg:text-center">
                Search Courses
              </div>
              <form
              ...
              >
                <input
            className="font-goldman pb-2 border rounded-[3px] border-[#D0D0D0] pt-[2px] px-3 outline-none"
            ...
            {cities.map((city) => (
              <option key={city} value={city}>
                ...
                <button
            type="submit"
            className="w-full h-[34px] flex-center text-blue font-goldman bg-yellow rounded-[3px] border border-yellow hover:bg-yellow/90 transition-colors"
                >
            Search
                </button>
              </form>
            ...
            <div className="shadow-custom rounded-[7px] border border-[#D0D0D0] overflow-hidden max-w-[592px] mx-auto mt-8 mb-6">
              <div className="bg-blue text-white font-goldman px-4 py-[6px] max-lg:text-center">
                Course Details
              </div>
              <div className="p-4">
                <div className="mb-3">
           ...
           {/* Gallery Section */}
          <div className="bg-red px-4 py-[7px] text-white shadow-custom font-goldman max-lg:text-center mt-3 mb-1">
            <h2>Gallery</h2>
          </div>
