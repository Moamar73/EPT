...
const Trainers = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_LINK}/api/instructors`)
      ...
      <div>
        <div className="container-px flex justify-between max-lg:flex-col gap-x-20 font-times pt-9">
            <div className="py-5 pb-16 px-2 xl:max-w-[650px] lg:max-w-[520px] max-w-full w-full">
                <div>
                    <h1 className="font-goldman mb-6 text-[#F9B223] xl:text-[35px] lg:text-[31px] xs:text-[35px] text-[30px]">Our Trainers</h1>
                    <p className="xl:text-[21px] lg:text-[17px] xs:text-[21px] text-[#545353]">
                        <span className="xl:text-[34px] lg:text-[32px] xs:text-[34px] text-[30px] text-[#BBB8B9]">At </span>
            ...
            <div className="flex justify-center w-full items-end xl:max-w-[600px] lg:max-w-[440px]">
                <picture>
                    ...
                    <img className="max-xs:max-h-[400px]" src="imgs/Trainers/trainer-0.png" alt="trainer-0" />
                </picture>
            </div>
        </div>      </div>

      <div className="container-px bg-[#F4F3F3] grid lg:grid-cols-2 gap-x-4 max-xs:gap-y-4 pt-14 lg:pb-[130px] md:pb-[100px] pb-[70px]">
          {trainers.map((trainer, i) => {
              // Checkerboard pattern logic
              const isReversed = (Math.floor(i / 2) + i % 2) % 2 === 1;
          
              return (
                  <div className={`flex rounded-[9px] border-4 border-white overflow-hidden even:max-lg:flex-row-reverse max-xs:!flex-col ${isReversed ? "lg:flex-row-reverse" : ""}`} key={trainer._id || i}>
                      <div className={`xs:w-1/2 border-white box-content max-xs:border-b-[5px] ${isReversed ? "xs:border-l-[5px]" : "xs:border-r-[5px]"}`}>
                      ...
                      <div className="py-5 px-4 xs:w-1/2">
                          <h4 className="font-goldman font-bold lg:text-[17px] mb-4 sm:text-[22px] xs:text-[16px] text-[18px]">{trainer.name}</h4>
                          <p className="lg:text-[16px] sm:text-[20px] xs:text-[14px] text-[16px]">{trainer.short_description}</p>
                      </div>
                  </div>
              );
          })}
