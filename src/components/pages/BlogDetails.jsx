...
// Default blog image
import defaultBlogImage from '../../assets/blogs/article1.png';
...
const BlogDetails = () => {
  ...
  // Get blog details from navigation state (passed from Blogs component)
  const blogDetailsFromState = location.state?.blogDetails;
  ...
  const fetchBlogDetails = async (blogId) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_LINK}/api/blogs/${blogId}`);
  ...
  if (error) {
    return (
      <div className='container-px py-20'>        <div className='text-center py-20'>
          <p className='text-red-500 mb-4'>{error}</p>
          <div className='space-x-4'>
            {id && (
              <button
                onClick={() => fetchBlogDetails(id)}
                className='px-6 py-2 bg-blue text-white rounded hover:bg-blue-700'
              >
                Retry
              </button>
            )}
    ...
    return (
      <div className='container-px py-20'>
        <div className='text-center py-20'>
          ...
          <button
            onClick={goBackToBlogs}
            className='px-6 py-2 bg-blue text-white rounded hover:bg-blue-700 mb-4'
          >
            Back to Blogs
          </button>
        </div>
