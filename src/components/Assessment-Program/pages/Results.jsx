...
import {
  resultsSections,
  behavioralCompetenciesResults,
  technicalCompetenciesResults,
  leadershipCompetenciesResults,
} from '../quizzData';
const Results = () => {
  const [currentSection, setCurrent] = useState(1);
  const [progress, setProgress] = useState(8);
  const [subsectionResults, setSubsectionResults] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const location = useLocation();
  const userString = localStorage.getItem('user');
  const user = userString ? JSON.parse(userString) : null;  useEffect(() => {
    if (!user?.user_id) {
  ...
  console.log('Render state:', { 
    isLoading, 
    hasResults: subsectionResults.length > 0, 
    resultsCount: subsectionResults.length 
  });

  // const totalScore = Number(
  //   (
  //     sectionAverages.reduce((sum, curr) => sum + curr.average, 0) /
      ...
      >
        ...
        <section className='py-14'>
          <div className='flex items-start gap-4 font-bold text-[29px] max-sm:text-[24px] max-xs:text-[20px]'>
            <h1>نتائج تقييم الموظف {user.first_name}</h1>
