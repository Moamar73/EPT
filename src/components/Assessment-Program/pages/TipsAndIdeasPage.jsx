...
const TipsAndIdeasPage = () => {
  ...
  useEffect(() => {
    const fetchSavedTips = async (userId) => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_LINK}/api/users/tips/${userId}`
        );
    ...
    const fetchTipsFromFinalResults = async (userId) => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_LINK}/api/sections/finalResults/${userId}`
        );
    ...
    const saveTipsToUser = async (userId, tipsString) => {
      try {
        await fetch(
          `${import.meta.env.VITE_API_LINK}/api/users/${userId}/saveTips`,
          {
            method: 'POST',
    ...
    const loadTips = async () => {
      ...
      let savedTipsString = await fetchSavedTips(user.user_id);
      ...
      if (savedTipsString && savedTipsString.trim() !== '') {
        ...
        const freshTips = await fetchTipsFromFinalResults(user.user_id);
        setTips(freshTips);
        await saveTipsToUser(user.user_id, freshTips.join('\n'));
        setLoading(false);
      }
    };
...
export default TipsAndIdeasPage;
