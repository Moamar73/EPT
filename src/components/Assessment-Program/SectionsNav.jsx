...
const SectionsNav = ({ currentSection, user, progress, sections }) => {
  // Helper to get title string if sections are objects
  const getTitle = (section) => (typeof section === "string" ? section : section.title);
  const sectionRoutes = {
  1: "/results",
  2: "/managerResults",
  3: "/finalResults",
      ...
      <div className="flex justify-between items-center max-[826px]:text-[14px] max-sm:text-[12px] text-center">
        <ul className="flex gap-7 max-[826px]:gap-2">
          ...
          {sections.ismanager === 1 || (user.role_id === 4 || user.role_id === 3) && (
            <li>
              <Link className="inline-block p-3 pt-1 max-sm:p-2 max-sm:pt-1" to={"/employees"} state={user}>
                تقييم الموظفين
              </Link>
            </li>
          )}
