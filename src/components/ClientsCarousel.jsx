...
const ClientsCarousel = ({componentTitle, componentEndpoint}) => {
  ...
  useEffect(() => {
    async function fetchClients() {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_LINK}/api/${componentEndpoint}`);
  ...
  return (
    <section className='container-px xl:py-[90px] lg:pt-[60px] lg:pb-[80px] py-[90px]'>
      <div>
        <h2 className='font-goldman lg:text-[45px] sm:text-[40px] text-[32px] text-center text-red mb-[65px]'>
          {componentTitle}
        </h2>
      </div>
